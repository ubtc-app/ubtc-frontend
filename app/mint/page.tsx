      ...
      <div style={{ maxWidth: '480px', margin: '0 auto', padding: '32px 20px' }}>
        ...
        {step === 'quantum' && (
          ...
          <label style={{ display: 'block', color: 'hsl(0 0% 35%)', fontSize: '11px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.12em', marginBottom: '8px' }}>Protocol Authorization Key</label>
            <button
              onClick={() => { const i = document.createElement('input'); i.type = 'file'; i.accept = '.json,.txt,.key'; i.onchange = (e: any) => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = ev => { try { const content = ev.target?.result as string; let k = ''; try { const j = JSON.parse(content); k = j?.protocol_second_key?.key || j?.protocol_second_key || ''; } catch { const match = content.match(/[a-f0-9]{64,}/); k = match ? match[0] : ''; } if (k) setSecondKey(k); else setError('Key not found in file'); } catch { setError('Invalid key file'); } }; r.readAsText(f); }; i.click(); }}
