    ...
    <div style={{ minHeight: '100vh', background: 'var(--t-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '480px' }}>
        ...
        <div style={{ textAlign: 'center' as const, marginBottom: '32px' }}>
          <h1 style={{ color: 'var(--t-text)', fontSize: '28px', fontWeight: '700', margin: '0 0 8px' }}>Redeem UBTC</h1>
          <p style={{ color: 'var(--t-faint)', fontSize: '13px', ...mono, margin: 0 }}>Convert your UBTC proof to Bitcoin</p>
        </div>
          ...
          {['Upload Proof', 'Unlock', 'Redeem', 'Done'].map((s, i) => {
            ...
            return (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: i <= current ? 'var(--t-green)' : 'var(--t-surface2)', border: `1px solid ${i <= current ? 'var(--t-green)' : 'var(--t-border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ color: i <= current ? 'white' : 'var(--t-faint)', fontSize: '11px', fontWeight: 700 }}>{i + 1}</span>
                </div>
                {i < 3 && <div style={{ width: '24px', height: '1px', background: i < current ? 'var(--t-green)' : 'var(--t-border)' }} />}
              </div>
            )
          })}
        </div>
        ...
        <div style={{ background: 'var(--t-surface)', border: '1px solid var(--t-border)', borderRadius: '20px', padding: '28px' }}>
          ...
          {step === 'password' && (
            <div>
              <p style={{ color: 'var(--t-text)', fontSize: '16px', fontWeight: 600, margin: '0 0 8px' }}>Enter your wallet password</p>
              <p style={{ color: 'var(--t-faint)', fontSize: '12px', ...mono, margin: '0 0 8px', lineHeight: '1.7' }}>
                Proof: <span style={{ color: 'var(--t-green)' }}>{proofFile?.proof_id}</span>
              </p>
              <p style={{ color: 'var(--t-faint)', fontSize: '12px', ...mono, margin: '0 0 20px', lineHeight: '1.7' }}>
                Amount: <span style={{ color: 'var(--t-orange)' }}>{proofFile?.ownership?.ubtc_amount} UBTC</span>
              </p>
              <input
                type="password"
             ...
             <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: '12px 0 0', textAlign: 'center' as const }}>
  Forgot password? <a href="/reset-password" style={{ color: 'var(--t-accent)', textDecoration: 'none' }}>Reset using recovery phrase →</a>
</p>
            </div>
          )}
          ...
          {step === 'redeem' && (
            <div>
              <p style={{ color: 'var(--t-text)', fontSize: '16px', fontWeight: 600, margin: '0 0 8px' }}>Enter your Bitcoin address</p>
              <p style={{ color: 'var(--t-faint)', fontSize: '12px', ...mono, margin: '0 0 20px', lineHeight: '1.7' }}>
                Redeeming <span style={{ color: 'var(--t-orange)' }}>{proofFile?.ownership?.ubtc_amount} UBTC</span> — BTC will be sent to this address.
              </p>
              <input
                type="text"
          ...
          {step === 'done' && result && (
            <div style={{ textAlign: 'center' as const }}>
              ...
              <div style={{ background: 'var(--t-surface)', borderRadius: '10px', padding: '16px', textAlign: 'left' as const }}>
                ...
                <p style={{ color: 'var(--t-green)', fontSize: '14px', fontWeight: 700, ...mono, margin: 0 }}>{result.amount_sats?.toLocaleString()} sats</p>
              </div>
              <a href={`https://mempool.space/testnet4/tx/${result.txid}`} target="_blank" rel="noopener noreferrer" style={{ display: 'block', marginTop: '16px', color: 'var(--t-accent)', fontSize: '12px', ...mono }}>
