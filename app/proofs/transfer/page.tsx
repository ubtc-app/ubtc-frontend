  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--t-bg)', fontFamily: 'var(--font-display)' }}>
      ...
      <div style={{ background: 'var(--t-surface)', borderBottom: '1px solid var(--t-border-subtle)', height: '60px', display: 'flex', alignItems: 'center', padding: '0 24px', gap: '16px' }}>
        <a href="/wallet" style={{ color: 'var(--t-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', ...mono }}>← Wallet</a>
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', gap: '8px', alignItems: 'center' }}>
          {steps.filter(s => s !== 'done').map((s, i) => (
            <div key={s} style={{ width: stepIdx === i ? '24px' : '8px', height: '8px', borderRadius: '4px', background: i <= stepIdx ? 'var(--t-accent)' : 'var(--t-border)', transition: 'all 0.3s' }} />
          ))}
        </div>
      ...
      <div style={{ maxWidth: '480px', margin: '0 auto', padding: '36px 20px' }}>
        ...
        <div style={{ textAlign: 'center' as const, marginBottom: '28px' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'var(--t-accent-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
          ...
          <h1 style={{ color: 'var(--t-text)', fontSize: '26px', fontWeight: '700', margin: '0 0 6px' }}>Transfer Proof</h1>
          <p style={{ color: 'var(--t-faint)', fontSize: '13px', ...mono, margin: 0 }}>Re-encrypts locally — your KEY 3 never leaves this device</p>
        </div>
        ...
        {step !== 'done' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'hsl(205 85% 55% / 0.06)', border: '1px solid hsl(205 85% 55% / 0.15)', borderRadius: '10px', padding: '10px 14px', marginBottom: '20px' }}>
            ...
            <p style={{ color: 'var(--t-accent)', fontSize: '12px', ...mono, margin: 0 }}>
        ...
        {/* Error */}
        {error && (
          <div style={{ background: 'var(--t-red-bg)', border: '1px solid hsl(0 84% 60% / 0.3)', borderRadius: '12px', padding: '12px 14px', marginBottom: '16px', display: 'flex', gap: '8px' }}>
            <span style={{ color: 'var(--t-red)', flexShrink: 0 }}>⚠</span>
        ...
        {step === 'key' && proofFile && (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
            {/* Proof summary */}
            <div style={{ background: 'var(--t-surface)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '16px', padding: '16px 18px' }}>
              <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.15em', margin: '0 0 8px' }}>Proof to transfer</p>
              <p style={{ color: 'var(--t-accent)', fontSize: '12px', ...mono, margin: '0 0 6px', wordBreak: 'break-all' as const }}>{proofFile.proof_id}</p>
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <p style={{ color: 'var(--t-text)', fontWeight: '700', fontSize: '18px', ...mono, margin: 0 }}>{proofFile.ownership?.ubtc_amount} UBTC</p>
                <span style={{ background: 'var(--t-accent-bg)', color: 'var(--t-accent)', fontSize: '11px', fontWeight: '600', padding: '3px 10px', borderRadius: '20px', ...mono }}>
                  Hop {(proofFile.ownership_chain?.length || 0) + 1}
                </span>
              </div>
              {proofFile.ownership_chain?.length > 0 && (
                <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: '6px 0 0' }}>
                  {proofFile.ownership_chain.length} previous transfer{proofFile.ownership_chain.length > 1 ? 's' : ''} in chain
                </p>
              )}
            </div>

            <p style={{ color: 'var(--t-muted)', fontSize: '12px', ...mono, margin: 0 }}>Your KEY 3 — needed to decrypt and re-encrypt for the recipient</p>

            {/* Upload key file */}
            <button
              onClick={() => keyFileRef.current?.click()}
              style={{ display: 'flex', alignItems: 'center', gap: '14px', background: kyberKey ? 'hsl(142 76% 36% / 0.07)' : 'var(--t-surface)', border: `1px solid ${kyberKey ? 'hsl(142 76% 36% / 0.4)' : 'var(--t-border)'}`, borderRadius: '14px', padding: '16px', cursor: 'pointer', width: '100%', textAlign: 'left' as const, fontFamily: 'var(--font-display)' }}
            >
            ...
            <textarea
              style={{ display: 'block', width: '100%', padding: '13px 16px', background: 'var(--t-surface)', border: `1px solid ${kyberKey ? 'hsl(142 76% 36% / 0.4)' : 'var(--t-border)'}`, borderRadius: '12px', color: 'var(--t-green)', fontSize: '11px', fontFamily: 'var(--font-mono)', outline: 'none', boxSizing: 'border-box' as const, resize: 'none' as const, height: '80px', lineHeight: '1.6' }}
            ...
            <button
              ...
              style={{ width: '100%', padding: '14px', borderRadius: '14px', border: 'none', background: loading || (!kyberKey && isEncrypted) ? 'var(--t-border)' : 'var(--t-accent)', color: loading || (!kyberKey && isEncrypted) ? 'var(--t-faint)' : 'white', fontSize: '15px', fontWeight: '700', cursor: loading || (!kyberKey && isEncrypted) ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-display)' }}
            >
        ...
        {step === 'recipient' && (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
            <div style={{ background: 'hsl(142 76% 36% / 0.07)', border: '1px solid hsl(142 76% 36% / 0.3)', borderRadius: '14px', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <p style={{ color: 'var(--t-green)', fontSize: '13px', fontWeight: '700', margin: '0 0 2px' }}>Decrypted locally</p>
                <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: 0 }}>KEY 3 was not sent to any server</p>
              </div>
            </div>

            <div>
              <label style={{ display: 'block', color: 'var(--t-faint)', fontSize: '11px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.12em', marginBottom: '8px' }}>Recipient — username or wallet address</label>
              <input
                ...
                autoFocus
                style={{ display: 'block', width: '100%', padding: '13px 16px', background: 'var(--t-surface)', border: '1px solid var(--t-border)', borderRadius: '12px', color: 'var(--t-text)', fontSize: '14px', fontFamily: 'var(--font-mono)', outline: 'none', boxSizing: 'border-box' as const, marginBottom: '10px' }}
              />
              <button
            ...
            {recipientWallet && (
              <>
                <div style={{ background: 'var(--t-surface)', border: '1px solid hsl(142 76% 36% / 0.3)', borderRadius: '14px', padding: '16px 18px' }}>
                  <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.15em', margin: '0 0 10px' }}>Recipient found</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                    <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'var(--t-accent-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: 'var(--t-accent)', fontSize: '16px', fontWeight: '700' }}>
                        {(recipientWallet.username || '?')[0].toUpperCase()}
                      </span>
                    </div>
                    <div>
                      <p style={{ color: 'var(--t-text)', fontWeight: '700', fontSize: '14px', margin: '0 0 2px' }}>@{recipientWallet.username}</p>
                      <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: 0 }}>{recipientWallet.wallet_address?.slice(0, 20)}...</p>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'hsl(142 76% 36% / 0.06)', border: '1px solid hsl(142 76% 36% / 0.2)', borderRadius: '8px', padding: '7px 10px' }}>
                    <span style={{ color: 'var(--t-green)', fontSize: '12px' }}>✅</span>
                    <p style={{ color: 'var(--t-green)', fontSize: '11px', ...mono, margin: 0 }}>Has valid Kyber1024 public key — proof can be re-encrypted</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  style={{ width: '100%', padding: '14px', borderRadius: '14px', border: 'none', background: loading ? 'var(--t-border)' : 'linear-gradient(135deg, hsl(205,85%,55%), hsl(190,80%,50%))', color: loading ? 'var(--t-faint)' : 'white', fontSize: '15px', fontWeight: '700', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-display)' }}
                >
        ...
        {step === 'confirm' && newProof && (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '14px' }}>
            <div style={{ background: 'var(--t-surface)', border: '1px solid var(--t-border-subtle)', borderRadius: '16px', padding: '18px' }}>
              <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.15em', margin: '0 0 14px' }}>Transfer Summary</p>
              {[
                ...
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid var(--t-border-subtle)' }}>
                  <p style={{ color: 'var(--t-faint)', fontSize: '12px', ...mono, margin: 0 }}>{row.label}</p>
            ...
            <div style={{ background: 'hsl(38 92% 50% / 0.05)', border: '1px solid hsl(38 92% 50% / 0.2)', borderRadius: '12px', padding: '12px 14px' }}>
              <p style={{ color: 'var(--t-muted)', fontSize: '12px', ...mono, margin: 0, lineHeight: '1.7' }}>
            ...
            <button
              ...
              disabled={loading}
              style={{ width: '100%', padding: '14px', borderRadius: '14px', border: 'none', background: loading ? 'var(--t-border)' : 'linear-gradient(135deg, hsl(205,85%,55%), hsl(190,80%,50%))', color: loading ? 'var(--t-faint)' : 'white', fontSize: '15px', fontWeight: '700', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-display)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            >
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', color: 'var(--t-faint)', fontSize: '12px', ...mono, cursor: 'pointer', textAlign: 'center' as const }}
            >
              ← Change recipient
            </button>
          </div>
        ...
        {/* ── STEP 5 — Done ── */}
        {step === 'done' && result && (
          <div style={{ background: 'var(--t-surface)', border: '1px solid hsl(142 76% 36% / 0.3)', borderRadius: '20px', padding: '40px 24px', textAlign: 'center' as const }}>
            <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: 'var(--t-green-bg)', border: '2px solid hsl(142 76% 36% / 0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: '32px' }}>✅</div>
            <h2 style={{ color: 'var(--t-text)', fontSize: '22px', fontWeight: '700', margin: '0 0 8px' }}>Proof Transferred</h2>
            <p style={{ color: 'var(--t-muted)', fontSize: '13px', ...mono, margin: '0 0 6px', lineHeight: '1.6' }}>
              {proofFile?.ownership?.ubtc_amount} UBTC proof sent to @{recipientWallet?.username}
            </p>
            <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: '0 0 24px' }}>
              They will see it in their wallet to download. KEY 3 was never sent to any server.
            </p>
            <div style={{ background: 'var(--t-surface)', borderRadius: '10px', padding: '12px 14px', marginBottom: '20px' }}>
              <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.1em', margin: '0 0 4px' }}>New proof ID</p>
              <p style={{ color: 'var(--t-green)', fontSize: '11px', ...mono, margin: 0, wordBreak: 'break-all' as const }}>{newProof?.proof_id}</p>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={() => { setStep('upload'); setProofFile(null); setKyberKey(''); setDecryptedTaprootKey(''); setRecipientInput(''); setRecipientWallet(null); setNewProof(null); setResult(null); setError('') }} style={{ flex: 1, background: 'var(--t-surface2)', border: '1px solid var(--t-border)', color: 'var(--t-muted)', borderRadius: '12px', padding: '13px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'var(--font-display)' }}>
                Transfer Another
              </button>
            </div>
          </div>
