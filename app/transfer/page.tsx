...
function TransferContent() {
 ...
 const [walletPassword, setWalletPassword] = useState('')
  ...
  const tokenColor = isUusdt ? 'var(--t-green)' : isUusdc ? 'var(--t-accent)' : 'var(--t-accent)'
    ...
    <div style={{ minHeight: '100vh', background: 'var(--t-bg)', fontFamily: 'var(--font-display)' }}>
      ...
      {showOwnWalletQuestion && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: 'var(--t-surface2)', border: '1px solid var(--t-border)', borderRadius: '24px', padding: '36px', maxWidth: '400px', width: '100%' }}>
            ...
            <button onClick={() => setShowOwnWalletQuestion(false)} style={{ width: '100%', background: 'none', border: 'none', color: 'var(--t-faint)', fontSize: '13px', ...mono, cursor: 'pointer', marginTop: '14px', padding: '8px' }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {showWarning && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: 'var(--t-surface2)', border: `2px solid ${isOwnWallet ? 'var(--t-accent-border)' : 'hsl(0 84% 60% / 0.4)'}`, borderRadius: '24px', padding: '32px', maxWidth: '420px', width: '100%', maxHeight: '90vh', overflowY: 'auto' as const }}>
            {isOwnWallet ? (
              <div>
                <div style={{ textAlign: 'center' as const, marginBottom: '20px' }}>
                  <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'var(--t-accent-bg)', border: '2px solid hsl(205 85% 55% / 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
                    {Icons.shield(28, 'var(--t-accent)')}
                  </div>
                  <h3 style={{ color: 'var(--t-text)', fontSize: '20px', fontWeight: '700', margin: '0 0 8px' }}>Your Collateral Stays With You</h3>
                  <p style={{ color: 'var(--t-muted)', fontSize: '13px', ...mono, margin: 0, lineHeight: '1.7' }}>
                ...
                <div onClick={() => setCollateralAcknowledged(!collateralAcknowledged)} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', cursor: 'pointer', padding: '12px', background: collateralAcknowledged ? 'var(--t-green-bg)' : 'var(--t-surface)', border: `1px solid ${collateralAcknowledged ? 'hsl(142 76% 36% / 0.4)' : 'var(--t-border)'}`, borderRadius: '10px', marginBottom: '18px' }}>
                  <div style={{ width: '20px', height: '20px', borderRadius: '5px', border: `2px solid ${collateralAcknowledged ? 'var(--t-green)' : 'var(--t-surface3)'}`, background: collateralAcknowledged ? 'var(--t-green)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {collateralAcknowledged && Icons.check(12, 'white')}
                  </div>
                  <p style={{ color: 'var(--t-muted)', fontSize: '12px', ...mono, margin: 0, lineHeight: '1.6' }}>I understand that if I send this {utokenName} to another person, they gain the right to the BTC collateral.</p>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button onClick={() => setShowWarning(false)} style={{ flex: 1, background: 'none', border: '1px solid var(--t-border)', color: 'var(--t-muted)', borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'var(--font-display)' }}>Cancel</button>
                  <button onClick={doSend} disabled={!collateralAcknowledged || loading} style={{ flex: 2, background: collateralAcknowledged ? 'linear-gradient(135deg, hsl(205,85%,55%), hsl(190,80%,50%))' : 'var(--t-border)', color: collateralAcknowledged ? 'white' : 'var(--t-faint)', border: 'none', borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: '700', cursor: collateralAcknowledged ? 'pointer' : 'not-allowed', fontFamily: 'var(--font-display)' }}>
              ...
              <div>
                ...
                <div style={{ background: 'hsl(0 84% 60% / 0.06)', border: '1px solid hsl(0 84% 60% / 0.25)', borderRadius: '14px', padding: '16px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                    ...
                    <p style={{ color: 'var(--t-muted)', fontSize: '12px', ...mono, margin: 0, lineHeight: '1.6' }}>{utokenName} is a claim on real Bitcoin. The recipient can redeem the BTC backing this token.</p>
                  </div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    {Icons.warning(14, 'var(--t-red)')}
                    <p style={{ color: 'var(--t-muted)', fontSize: '12px', ...mono, margin: 0, lineHeight: '1.6' }}>You cannot reverse this transaction once sent.</p>
                  </div>
                </div>
                <div style={{ background: 'var(--t-surface)', borderRadius: '12px', padding: '14px', marginBottom: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <div style={{ textAlign: 'center' as const }}>
                    <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase', margin: '0 0 5px' }}>Sending</p>
                    <p style={{ color: tokenColor, fontWeight: '700', fontSize: '15px', ...mono, margin: 0 }}>{parseFloat(amount || '0').toLocaleString()} {utokenName}</p>
                  </div>
                  <div style={{ textAlign: 'center' as const }}>
                    <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase', margin: '0 0 5px' }}>Collateral at Risk</p>
                    <p style={{ color: 'var(--t-red)', fontWeight: '700', fontSize: '15px', ...mono, margin: 0 }}>
                      {activeCurrency === 'ubtc' ? btcRelease + ' BTC' : '$' + parseFloat(amount || '0').toLocaleString() + ' ' + tokenName}
                    </p>
                  </div>
                </div>
                <div onClick={() => setCollateralAcknowledged(!collateralAcknowledged)} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', cursor: 'pointer', padding: '12px', background: collateralAcknowledged ? 'var(--t-red-bg)' : 'var(--t-surface)', border: `1px solid ${collateralAcknowledged ? 'hsl(0 84% 60% / 0.4)' : 'var(--t-border)'}`, borderRadius: '10px', marginBottom: '18px' }}>
                  <div style={{ width: '20px', height: '20px', borderRadius: '5px', border: `2px solid ${collateralAcknowledged ? 'var(--t-red)' : 'var(--t-surface3)'}`, background: collateralAcknowledged ? 'var(--t-red)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {collateralAcknowledged && Icons.check(12, 'white')}
                  </div>
                  <p style={{ color: 'var(--t-muted)', fontSize: '12px', ...mono, margin: 0, lineHeight: '1.6' }}>I understand the recipient gains the right to redeem the BTC. This is irreversible.</p>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button onClick={() => setShowWarning(false)} style={{ flex: 1, background: 'none', border: '1px solid var(--t-border)', color: 'var(--t-muted)', borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'var(--font-display)' }}>Cancel</button>
                  <button onClick={doSend} disabled={!collateralAcknowledged || loading} style={{ flex: 2, background: collateralAcknowledged ? 'hsl(0 84% 55%)' : 'var(--t-border)', color: collateralAcknowledged ? 'white' : 'var(--t-faint)', border: 'none', borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: '700', cursor: collateralAcknowledged ? 'pointer' : 'not-allowed', fontFamily: 'var(--font-display)' }}>
      ...
      <div style={{ background: 'var(--t-surface)', borderBottom: '1px solid var(--t-border-subtle)', height: '60px', display: 'flex', alignItems: 'center', padding: '0 24px', gap: '16px' }}>
        ...
        <button onClick={loadAll} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--t-faint)' }}>
      ...
      <div style={{ maxWidth: '480px', margin: '0 auto', padding: '28px 16px' }}>
        {result ? (
          <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '24px' }}>
            <div style={{ width: '88px', height: '88px', borderRadius: '50%', background: 'var(--t-green-bg)', border: '2px solid hsl(142 76% 36% / 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {Icons.check(40, 'var(--t-green)')}
            </div>
            <div style={{ textAlign: 'center' as const }}>
            ...
            <div style={{ width: '100%', background: 'var(--t-surface)', borderRadius: '20px', padding: '22px' }}>
              {[
            ...
            {result.bitcoin_txid && (
              <div style={{ background: 'var(--t-surface)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '12px', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                <div>
                  <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.1em', margin: '0 0 4px' }}>QUANTUM:TRANSFER on Bitcoin</p>
                  <p style={{ color: 'var(--t-accent)', fontSize: '11px', ...mono, margin: 0, wordBreak: 'break-all' as const }}>{result.bitcoin_txid}</p>
                </div>
                <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                  ...
                  }} style={{ background: 'var(--t-surface2)', color: 'var(--t-muted)', border: '1px solid var(--t-border)', borderRadius: '8px', padding: '8px 12px', fontSize: '11px', ...mono, cursor: 'pointer', whiteSpace: 'nowrap' as const }}>
                    {copied ? '✅ Copied!' : 'Copy URL'}
                  </button>
                  <a href={`https://mempool.space/testnet4/tx/${result.bitcoin_txid}`} target="_blank" rel="noopener noreferrer" style={{ background: 'var(--t-accent)', color: '#000', borderRadius: '8px', padding: '8px 12px', fontSize: '11px', ...mono, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap' as const }}>
                    View →
                  </a>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: '10px', width: '100%' }}>
              <button onClick={() => { setResult(null); setAmount(''); setSelectedWallet(null); setSearchQuery(''); setUserPassword('') }} style={{ flex: 1, background: 'var(--t-surface2)', border: '1px solid var(--t-border)', color: 'var(--t-muted)', borderRadius: '14px', padding: '16px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: 'var(--font-display)' }}>
                Send More
              </button>
          ...
          <div style={{ textAlign: 'center' as const, padding: '80px' }}>
            <p style={{ color: 'var(--t-faint)', fontSize: '14px', ...mono }}>Loading...</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
            ...
            {/* FROM */}
            {isWalletSend ? (
              <div style={{ background: 'var(--t-surface)', borderRadius: '20px', overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px 10px' }}>
                  <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase', letterSpacing: '0.2em', margin: '0 0 12px' }}>From Wallet</p>
                </div>
                <div style={{ padding: '4px 10px 10px' }}>
                  {[
                    { cur: 'ubtc', icon: Icons.bitcoin, color: 'var(--t-accent)', name: 'UBTC', bal: walletUbtc },
                    { cur: 'uusdt', icon: Icons.lock, color: 'var(--t-green)', name: 'UUSDT', bal: walletUusdt },
                    { cur: 'uusdc', icon: Icons.lock, color: 'var(--t-accent)', name: 'UUSDC', bal: walletUusdc },
                    ...
                    return (
                      <div key={t.cur} onClick={() => { setActiveCurrency(t.cur); setAmount('') }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', background: isSel ? t.color + '0e' : 'transparent', border: `1px solid ${isSel ? t.color + '40' : 'transparent'}`, borderRadius: '14px', cursor: 'pointer', marginBottom: '4px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: 0 }}>Available balance</p>
                        </div>
                  ...
                  {walletUbtc === 0 && walletUusdt === 0 && walletUusdc === 0 && (
                    <p style={{ color: 'var(--t-faint)', fontSize: '14px', ...mono, margin: '10px', textAlign: 'center' as const }}>No tokens in wallet</p>
                  )}
                </div>
              </div>
              ...
              <div style={{ background: 'var(--t-surface)', borderRadius: '20px', overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px 10px' }}>
                  <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase', letterSpacing: '0.2em', margin: 0 }}>From Account</p>
                </div>
                <div style={{ padding: '4px 10px 10px' }}>
                  {fromOptions.map((opt, i) => {
                    ...
                    return (
                      <div key={i} onClick={() => setActiveVaultId(opt.vaultId)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', background: isSelected ? tokenColor + '0e' : 'transparent', border: `1px solid ${isSelected ? tokenColor + '40' : 'transparent'}`, borderRadius: '14px', cursor: 'pointer', marginBottom: '4px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <p style={{ color: 'var(--t-text)', fontWeight: '600', fontSize: '14px', margin: '0 0 2px' }}>{utokenName}</p>
                          <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: 0 }}>{opt.meta.title} · {opt.note}</p>
                        </div>
              ...
              <div style={{ background: 'var(--t-surface)', borderRadius: '20px', padding: '28px', textAlign: 'center' as const }}>
                <p style={{ color: 'var(--t-faint)', fontSize: '14px', ...mono, margin: '0 0 12px' }}>No {utokenName} balance to send</p>
                <a href={`/deposit?vault=${vaultId}&currency=${activeCurrency}`} style={{ color: tokenColor, fontSize: '13px', ...mono }}>Deposit {tokenName} first</a>
              </div>
            )}

            {/* AMOUNT */}
            <div style={{ background: 'var(--t-surface)', borderRadius: '20px', padding: '22px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase', letterSpacing: '0.2em', margin: 0 }}>Amount</p>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                {[25, 50, 75, 100].map(pct => (
                  <button key={pct} onClick={() => setAmount((activeBalance * pct / 100).toFixed(2))} style={{ flex: 1, background: 'var(--t-surface)', border: '1px solid var(--t-border-subtle)', color: 'var(--t-faint)', borderRadius: '9px', padding: '9px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', ...mono }}>{pct}%</button>
                ))}
              </div>
              {amount && parseFloat(amount) > activeBalance && (
                <p style={{ color: 'var(--t-red)', fontSize: '12px', ...mono, margin: '10px 0 0' }}>Exceeds balance</p>
              )}
            </div>

            {/* TO — search only, UBTC wallets only */}
            <div style={{ background: 'var(--t-surface)', borderRadius: '20px', padding: '20px' }}>
              <p style={{ color: 'var(--t-faint)', fontSize: '10px', ...mono, textTransform: 'uppercase', letterSpacing: '0.2em', margin: '0 0 6px' }}>To</p>
              ...
              <input
                ...
                style={{ width: '100%', padding: '13px 14px', background: 'var(--t-surface)', border: `1px solid ${searchError ? 'hsl(0 84% 60% / 0.6)' : (selectedWallet || isManualUbtcAddress) ? 'hsl(142 76% 36% / 0.5)' : 'var(--t-border)'}`, borderRadius: '12px', color: 'var(--t-text)', fontSize: '13px', fontFamily: 'var(--font-mono)', outline: 'none', boxSizing: 'border-box' as const }}
              />

              {searchError && (
                <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', background: 'var(--t-red-bg)', border: '1px solid hsl(0 84% 60% / 0.3)', borderRadius: '10px', padding: '10px 12px', marginTop: '8px' }}>
                  ...
                  {searchResults.map((w: any, i: number) => {
                    ...
                    return (
                      <div key={i} onClick={() => { setSelectedWallet(isSel ? null : w); if (!isSel) { setSearchQuery(w.wallet_address); fetchRecipientWallet(w.wallet_address) } }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', background: isSel ? 'hsl(205 85% 55% / 0.07)' : 'var(--t-surface)', border: `1px solid ${isSel ? 'var(--t-accent-border)' : 'var(--t-border-subtle)'}`, borderRadius: '12px', cursor: 'pointer', marginTop: '6px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <p style={{ color: 'var(--t-text)', fontWeight: '600', fontSize: '13px', margin: '0 0 2px' }}>@{w.username}</p>
              ...
              {isManualUbtcAddress && !selectedWallet && (
                <div style={{ marginTop: '8px', background: 'hsl(142 76% 36% / 0.06)', border: '1px solid hsl(142 76% 36% / 0.3)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <p style={{ color: 'var(--t-green)', fontSize: '11px', ...mono, margin: 0 }}>Valid UBTC address</p>
                </div>
              )}
            ...
            {!isWalletSend && !isStable && (
              <div style={{ background: 'var(--t-surface)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '20px', padding: '22px 24px' }}>
                ...
                <label style={{ display: 'block', width: '100%', background: 'var(--t-accent)', color: '#000', fontFamily: 'var(--font-mono)', fontSize: '11px', fontWeight: 700, padding: '10px 14px', borderRadius: '8px', cursor: 'pointer', textAlign: 'center' as const, marginBottom: '10px', boxSizing: 'border-box' as const }}>
                  📁 Load Key File
                  <input type="file" accept=".json" style={{ display: 'none' }} onChange={loadKeyFile} />
                </label>
                <textarea value={qsk} onChange={e => { setQsk(e.target.value); setQskError('') }} placeholder="Or paste your Quantum Signing Key..." rows={3} style={{ width: '100%', padding: '12px 14px', background: 'var(--t-surface)', border: `1px solid ${qskError ? 'hsl(0 84% 60% / 0.5)' : 'var(--t-border)'}`, borderRadius: '10px', color: 'hsl(0 0% 75%)', fontSize: '11px', fontFamily: 'var(--font-mono)', outline: 'none', resize: 'none' as const, boxSizing: 'border-box' as const }} />
            ...
            {isWalletSend && (
              <div style={{ background: 'var(--t-surface)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '16px', padding: '18px 20px' }}>
                ...
                {isInTelegram() && !walletPassword && (
                  <div>
                    <textarea
                      ...
                      rows={3}
                      style={{ width: '100%', padding: '12px 14px', background: 'var(--t-surface)', border: `1px solid ${passwordError ? 'hsl(0 84% 60% / 0.5)' : 'var(--t-border)'}`, borderRadius: '10px', color: 'var(--t-text)', fontSize: '12px', fontFamily: 'var(--font-mono)', outline: 'none', resize: 'none' as const, boxSizing: 'border-box' as const, marginBottom: '10px' }}
                    />
                    <button
                      onClick={async () => {
                      ...
                      style={{ width: '100%', background: pskPasteInput.trim() ? 'var(--t-accent)' : 'var(--t-border)', color: pskPasteInput.trim() ? '#000' : 'var(--t-faint)', border: 'none', borderRadius: '8px', padding: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'var(--font-mono)', cursor: pskPasteInput.trim() ? 'pointer' : 'not-allowed' }}
                    >
                      Verify Key
                    </button>
                  </div>
                ...
                {isInTelegram() && walletPassword && (
                  <div style={{ width: '100%', background: 'var(--t-green-bg)', color: 'var(--t-green)', fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 700, padding: '12px 14px', borderRadius: '8px', textAlign: 'center' as const, boxSizing: 'border-box' as const, border: '1px solid hsl(142 76% 36% / 0.3)' }}>
                    ✅ Protocol Key Verified
                  </div>
                )}
                {passwordError && <p style={{ color: 'var(--t-red)', fontSize: '12px', ...mono, margin: '8px 0 0' }}>{passwordError}</p>}
                ...
                {walletPassword && (
                  <div style={{ marginTop: '14px', paddingTop: '14px', borderTop: '1px solid var(--t-border)' }}>
                    <p style={{ color: 'var(--t-accent)', fontSize: '10px', ...mono, textTransform: 'uppercase' as const, letterSpacing: '0.2em', margin: '0 0 4px' }}>🔐 Wallet Password</p>
                    <p style={{ color: 'var(--t-faint)', fontSize: '11px', ...mono, margin: '0 0 10px', lineHeight: '1.6' }}>Enter your wallet password to PQ-sign this transfer.</p>
                    <div style={{ position: 'relative' as const }}>
                      <input
                        ...
                        style={{ width: '100%', padding: '12px 44px 12px 14px', background: 'var(--t-surface)', border: `1px solid ${userPassword ? 'hsl(142 76% 36% / 0.4)' : 'var(--t-border)'}`, borderRadius: '10px', color: 'var(--t-text)', fontSize: '13px', fontFamily: 'var(--font-mono)', outline: 'none', boxSizing: 'border-box' as const }}
                      />
                      <button
                        type="button"
                        onClick={() => setShowUserPassword(!showUserPassword)}
                        style={{ position: 'absolute' as const, right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--t-muted)', fontSize: '11px', cursor: 'pointer', padding: '6px 10px', ...mono }}
                      >
            ...
            {error && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--t-red-bg)', border: '1px solid hsl(0 84% 60% / 0.25)', borderRadius: '14px', padding: '14px 16px' }}>
                {Icons.warning(14, 'var(--t-red)')}
