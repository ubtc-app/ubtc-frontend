...
export function QuantumLoader({ text = 'Loading' }: { text?: string }) {
  return (
    <div style={{
      ...
      justifyContent: 'center', background: 'var(--q-bg)',
    }}>
      <div style={{
        ...
        boxShadow: '0 0 60px rgba(0,212,255,0.1), 0 24px 64px rgba(0,0,0,0.7), inset 0 1px 0 rgba(0,212,255,0.12)',
      }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <p style={{ color: 'rgba(0,212,255,0.7)', fontSize: 11, fontFamily: 'var(--font-mono)', letterSpacing: '0.3em', textTransform: 'uppercase', margin: 0 }}>{text}</p>
          <span style={{ display: 'flex', gap: 3 }}>
            {[0,1,2].map(i => (
              <span key={i} style={{ width: 3, height: 3, borderRadius: '50%', background: '#00D4FF', opacity: 0.6, animation: `ql-dot 1.2s ease infinite`, animationDelay: `${i * 0.2}s` }} />
            ))}
          </span>
        </div>
        <style>{`
...
export function QuantumLoaderInline({ text = 'Loading' }: { text?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <span style={{ color: 'rgba(0,212,255,0.6)', fontSize: 11, fontFamily: 'var(--font-mono)', letterSpacing: '0.2em', textTransform: 'uppercase' }}>{text}</span>
