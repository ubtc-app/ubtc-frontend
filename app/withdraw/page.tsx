  ...
  return (
    <div style={{ minHeight: '100vh', background: 'hsl(220 15% 5%)', padding: '40px 24px', fontFamily: 'var(--font-display)' }}>
      <div style={{ maxWidth: '560px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: '40px', fontWeight: '700', lineHeight: '1', marginBottom: '12px', color: 'hsl(0 0% 92%)' }}>Withdraw</h1>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '32px', alignItems: 'center' }}>
          {[{ n: 1, label: 'Details' }, { n: 2, label: 'Confirm' }, { n: 3, label: 'Authorize' }, { n: 4, label: 'Done' }].map((s, i) => (
            <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '28px', height: '28px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: (step === 'form' && s.n === 1) || (step === 'warning' && s.n === 2) || (step === 'verify' && s.n === 3) || (step === 'done' && s.n === 4) ? 'hsl(205 85% 55%)' : 'hsl(220 12% 8%)', border: '1px solid hsl(220 10% 16%)', fontSize: '12px', fontWeight: '700', color: 'white' }}>{s.n}</div>
        ...
        <div style={{ background: 'hsl(220 12% 8%)', border: '1px solid hsl(220 10% 16%)', borderRadius: '16px', padding: '28px' }}>
          ...
          {step === 'form' && (
            <>
              ...
              <p style={{ color: 'hsl(0 0% 65%)', fontSize: '11px', ...mono, textTransform: 'uppercase', letterSpacing: '0.15em', margin: '0 0 10px' }}>Where to send?</p>
              <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                <div onClick={() => { setDestType('wallet'); setDestination('') }} style={{ flex: 1, background: destType === 'wallet' ? 'hsl(205 85% 55% / 0.1)' : 'hsl(220 15% 5%)', border: `2px solid ${destType === 'wallet' ? 'hsl(205 85% 55%)' : 'hsl(220 10% 16%)'}`, borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                  <p style={{ color: destType === 'wallet' ? 'hsl(205 85% 55%)' : 'hsl(0 0% 92%)', fontWeight: '600', fontSize: '13px', margin: '0 0 4px' }}>🔒 My UBTC Wallet</p>
                  <p style={{ color: 'hsl(0 0% 55%)', fontSize: '11px', ...mono, margin: 0, lineHeight: '1.5' }}>BTC stays locked. UBTC moves to wallet.</p>
                </div>
                <div onClick={() => { setDestType('btc'); setDestination('') }} style={{ flex: 1, background: destType === 'btc' ? 'hsl(38 92% 50% / 0.1)' : 'hsl(220 15% 5%)', border: `2px solid ${destType === 'btc' ? 'hsl(38 92% 50%)' : 'hsl(220 10% 16%)'}`, borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                  <p style={{ color: destType === 'btc' ? 'hsl(38 92% 50%)' : 'hsl(0 0% 92%)', fontWeight: '600', fontSize: '13px', margin: '0 0 4px' }}>₿ Bitcoin Address</p>
                  <p style={{ color: 'hsl(0 0% 55%)', fontSize: '11px', ...mono, margin: 0, lineHeight: '1.5' }}>Burns UBTC and releases BTC.</p>
                </div>
              </div>
              ...
              {destType === 'wallet' && (
                <>
                  {vaultWallets.length > 0 && (
                    <>
                      <label style={{ display: 'block', fontSize: '11px', ...mono, color: 'hsl(0 0% 65%)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '8px' }}>Select Linked Wallet</label>
                  ...
                  {vaultWallets.length === 0 && (
                    <p style={{ color: 'hsl(0 0% 45%)', fontSize: '11px', ...mono, margin: '-12px 0 16px', lineHeight: '1.6' }}>
                      No linked wallets found. Enter your wallet address above or{' '}
                      <a href={`/wallet?vault=${vaultId}`} style={{ color: 'hsl(205 85% 55%)' }}>create a linked wallet →</a>
                    </p>
                  )}
                  {destination && (
                    <div style={{ background: 'hsl(205 85% 55% / 0.05)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '10px', padding: '12px 14px', marginBottom: '20px' }}>
                      <p style={{ color: 'hsl(205 85% 55%)', fontSize: '12px', ...mono, margin: 0 }}>🔒 BTC stays locked. Only UBTC moves to your wallet. Collateral unchanged.</p>
                    </div>
                  )}
                </>
              )}
                ...
                <>
                  {btcToRelease > 0 && (
                    <div style={{ background: 'hsl(220 15% 5%)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '10px', padding: '14px 16px', marginBottom: '20px' }}>
                      <p style={{ color: 'hsl(0 0% 65%)', fontSize: '10px', ...mono, textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 4px' }}>BTC to Release</p>
          ...
          {step === 'warning' && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
                ...
                <h2 style={{ color: destType === 'wallet' ? 'hsl(205 85% 55%)' : 'hsl(38 92% 50%)', fontSize: '18px', fontWeight: '700', margin: 0 }}>
                ...
                {destType === 'wallet' ? (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>UBTC moving to wallet</span>
                      <span style={{ color: 'hsl(205 85% 55%)', fontWeight: '700', ...mono }}>${withdrawAmount.toLocaleString()} UBTC</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>Wallet</span>
                      <span style={{ color: 'hsl(0 0% 92%)', fontWeight: '700', ...mono, fontSize: '11px' }}>{destination.slice(0, 20)}...</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>BTC in vault after</span>
                      <span style={{ color: 'hsl(142 76% 36%)', fontWeight: '700', ...mono }}>{btcLocked.toFixed(4)} BTC — unchanged</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>Vault UBTC after</span>
                      <span style={{ color: 'hsl(0 0% 92%)', fontWeight: '700', ...mono }}>${ubtcRemainingAfter.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                    </div>
                  </>
                ) : (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>UBTC to burn</span>
                      <span style={{ color: 'hsl(0 84% 60%)', fontWeight: '700', ...mono }}>${withdrawAmount.toLocaleString()} UBTC</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>BTC to release</span>
                      <span style={{ color: 'hsl(205 85% 55%)', fontWeight: '700', ...mono }}>{btcToRelease.toFixed(6)} BTC</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>Destination</span>
                      <span style={{ color: 'hsl(0 0% 92%)', fontWeight: '700', ...mono, fontSize: '11px' }}>{destination.slice(0, 16)}...</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>UBTC remaining</span>
                      <span style={{ color: 'hsl(0 0% 92%)', fontWeight: '700', ...mono }}>${ubtcRemainingAfter.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
                      <span style={{ color: 'hsl(0 0% 55%)', fontSize: '12px', ...mono }}>BTC remaining in vault</span>
                      <span style={{ color: 'hsl(0 0% 92%)', fontWeight: '700', ...mono }}>{btcRemainingAfter.toFixed(6)} BTC</span>
                    </div>
                  </>
                )}
              </div>
              ...
              {destType === 'wallet' && (
                <div style={{ background: 'hsl(205 85% 55% / 0.05)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '10px', padding: '14px', marginBottom: '24px' }}>
                  <p style={{ color: 'hsl(205 85% 55%)', fontSize: '12px', ...mono, margin: 0, lineHeight: '1.7' }}>
          ...
          {step === 'done' && result && (
            <>
              <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                ...
                <p style={{ color: 'hsl(0 0% 65%)', fontSize: '12px', ...mono, textTransform: 'uppercase', letterSpacing: '0.15em', margin: '0 0 8px' }}>
                  {result.wallet_transfer ? 'Sent to Wallet' : 'Withdrawal Complete'}
                </p>
                <p style={{ fontSize: '32px', fontWeight: '700', color: 'hsl(205 85% 55%)', margin: 0 }}>${withdrawAmount.toLocaleString()} UBTC</p>
              </div>
              ...
              {result.wallet_transfer ? (
                <div style={{ background: 'hsl(205 85% 55% / 0.05)', border: '1px solid hsl(205 85% 55% / 0.2)', borderRadius: '10px', padding: '16px', marginBottom: '16px' }}>
                  <p style={{ color: 'hsl(205 85% 55%)', fontSize: '13px', fontWeight: '600', ...mono, margin: '0 0 8px' }}>🔒 Wallet Transfer Complete</p>
                  <p style={{ color: 'hsl(0 0% 65%)', fontSize: '12px', ...mono, margin: '0 0 4px' }}>UBTC moved to your wallet. BTC collateral remains fully locked.</p>
                  <p style={{ color: 'hsl(0 0% 55%)', fontSize: '11px', ...mono, margin: 0 }}>New wallet balance: ${parseFloat(result.new_wallet_balance || '0').toLocaleString()} UBTC</p>
                </div>
              ) : (
                <div style={{ background: 'hsl(220 15% 5%)', borderRadius: '10px', padding: '14px', marginBottom: '12px' }}>
                  {[
                    ...
                    <div key={item.label} style={{ padding: '8px 0', borderBottom: '1px solid hsl(220 10% 14%)' }}>
                      ...
                      <p style={{ color: 'hsl(0 0% 92%)', fontSize: '12px', ...mono, margin: 0, wordBreak: 'break-all' as const }}>{item.value}</p>
                    </div>
                  ))}
                </div>
              )}
